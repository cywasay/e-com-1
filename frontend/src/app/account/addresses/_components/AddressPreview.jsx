...
export default function AddressPreview({ user, isPending }) {
  ...
  return (
    <div className="space-y-8">
      ...
      {isPending || !user ? <LoadingPreview /> : address ? (
        <div className="p-8 rounded-3xl bg-white shadow-sm border border-[#e8e4dc] space-y-6 relative overflow-hidden">
          <div className="space-y-4">
            <div><p className="text-sm font-bold text-[#1a1a2e]">{address.full_name}</p><p className="text-sm text-[#6b6560] mt-1">{address.phone}</p></div>
            <div className="text-sm text-[#1a1a2e] leading-relaxed">
              <p>{address.address_line_1}</p>
              {address.address_line_2 && <p>{address.address_line_2}</p>}
              <p>{address.city}, {address.emirates}</p>
              <p className="font-bold mt-2 uppercase tracking-widest text-[10px]">{address.country}</p>
            </div>
          </div>
